...
Сценарий: <ПодключениеУслугУзлы>
    И я подготавливаю тестовые данные
        И я загружаю макет "ПодключениеУслугиУзлы.mxl"
    ...
    И я заполняю форму открывшегося документа     
        И из выпадающего списка "Абонент" я выбираю по строке 'Миронова Мария Петровна'
        И из выпадающего списка "Контроллер" я выбираю по строке 'Войкова Е.С.'
        Тогда открылось окно 'Подключение/отключение услуг (создание) *'
        И в таблице "ТочкиУчета" я нажимаю на кнопку с именем 'ТочкиУчетаЕстьШкалыЕстьТарифныеЗоныДобавитьТочкуСУзлом1'
	...
	И я заполняю строку табличной части	
	    И из выпадающего списка "Услуга" я выбираю по строке 'Электроэнергия'
...
# Технические свойства : "Выбраны"
    И я заполняю форму открывшегося документа  
	    ...
	    И в таблице "Измерители" я активизирую поле "Расположение измерителя"
	    И в таблице "Измерители" виз выпадающего списка  'Расположение измерителя' я выбираю по строке 'ввод'
        И в таблице "Измерители" я активизирую поле "Дата выпуска"
	    И в таблице "Измерители" в поле 'Дата выпуска' я ввожу текст '01.01.2020'
	    И в таблице "Измерители" я активизирую поле "Дата поверки"
	    И в таблице "Измерители" в поле 'Дата поверки' я ввожу текст '01.01.2025'
	    ...
	    И я нажимаю на кнопку 'Завершить'
	    #Тогда открылось окно 'Подключение/отключение услуг (создание)'
	    Тогда в таблице "ТочкиУчета" количество строк "равно" 1
       # Тогда значение поля с именем "Услуга" содержит текст "Электроэнергия "
        ...
        И в таблице "ВидыБлагоустройстваТарифныеКатегории" из выпадающего списка "Тарифная категория" я выбираю по строке 'Многоквартирные дома, общежития квартирного типа, не оборудованные в установленном порядке стационарными электроплитами для приготовления пищи, электр'
    ...
    И я проверяю заполнение списка документов    
        ...
        | 'Миронова Мария Петровна' | Подключение услуги | '1-я Красноармейская ул, Дом № 2/23'  | Волховский район  |