...
Сценарий: Я запускаю выполнение начислений ОТЭК по организации "яОрганизация" и району "яРайон" и МКД "яМКД" с "ПериодНачало" "ГодНачало" по "ПериодОкончание"
    ...
    И Если выражение внутреннего языка "Не ПустаяСтрока(контекст.МКД)" истинно
        Тогда из выпадающего списка 'Дом' я выбираю точное значение "яМКД"
    И Если выражение внутреннего языка "ПустаяСтрока(контекст.МКД)" истинно
        Тогда я устанавливаю флаг "ОтборПоДому"
