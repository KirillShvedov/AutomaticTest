...
Функция ПолучитьМакетОбработки(ИмяФайла) Экспорт
	КаталогТеста	 	= "relative_step_definitions";
	ИмяФайлаМакета 		= "";
	
	КаталогПроекта 		= Ванесса.Объект.КаталогПроекта;
	Если НЕ Прав(КаталогПроекта,1) = "\" Тогда
		КаталогПроекта = КаталогПроекта + "\";
	КонецЕсли;
		
	Если Лев(ИмяФайла,2) = ".\" Тогда
		ИмяФайлаМакета 	= КаталогПроекта + Прав(КаталогТеста,СтрДлина(КаталогТеста)-2) + "\ИсходныеДанные\" + Прав(ИмяФайла,СтрДлина(ИмяФайла)-2);
	Иначе
		ИмяФайлаМакета 	= КаталогПроекта + Прав(КаталогТеста,СтрДлина(КаталогТеста)-2) + "\ИсходныеДанные\" + ИмяФайла;
	КонецЕсли;
	...
	Если Не Файл.Существует() Тогда
		Сообщить("step_definitions.ПолучитьМакетОбработки: Не удалось найти файл " + ИмяФайлаМакета);		
...
Процедура ПередОкончаниемСценария() Экспорт 	
	...
	Если НЕ СтрНайти(ПараметрЗапуска,"НеВыполнятьОчисткуПослеТеста") = 0  Тогда
		Возврат;
	КонецЕсли;
