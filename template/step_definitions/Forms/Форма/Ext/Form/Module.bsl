...
Функция ПолучитьМакетОбработки(ИмяФайла) Экспорт
	...
	Если Лев(ИмяФайла,2) = ".\" Тогда
		ИмяФайлаМакета 	= Ванесса.КаталогПроекта + Прав(КаталогТеста,СтрДлина(КаталогТеста)-2) + "\ИсходныеДанные\" + Прав(ИмяФайла,СтрДлина(ИмяФайла)-2);
	Иначе
		ИмяФайлаМакета 	= Ванесса.КаталогПроекта + Прав(КаталогТеста,СтрДлина(КаталогТеста)-2) + "\ИсходныеДанные\" + ИмяФайла;
	КонецЕсли;
