...
Функция ПолучитьМакетОбработки(ИмяФайла) Экспорт
	КаталогТеста	 	= Новый Файл(Ванесса.ТекущийЗагружаемыйФичаФайл);
	ИмяФайлаМакета 		= "";
	Если Лев(ИмяФайла,2) = ".\" Тогда
		ИмяФайлаМакета 	= КаталогТеста.Путь + "ИсходныеДанные\" + Прав(ИмяФайла,СтрДлина(ИмяФайла)-2);
	Иначе
		ИмяФайлаМакета 	= КаталогТеста.Путь + "ИсходныеДанные\" + ИмяФайла;
	КонецЕсли;
